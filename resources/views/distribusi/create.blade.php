...
<div class="container-fluid">
    <h1 class="mb-4">Tambah Distribusi Bantuan</h1>
        ...
        <form action="{{ route('distribusi.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('pengajuan_id') is-invalid @enderror" id="pengajuan_id" name="pengajuan_id" required>
                    <option value="">Pilih Pengajuan (Menunggu/Disetujui)</option>
                    @foreach($pengajuans as $pengajuan)
                        <option value="{{ $pengajuan->id }}" {{ old('pengajuan_id') == $pengajuan->id ? 'selected' : '' }}>
                            ID: {{ $pengajuan->id }} - {{ $pengajuan->warga->nama }} ({{ $pengajuan->jenisBantuan->nama_bantuan }}) - Status: {{ ucfirst($pengajuan->status) }}
                        </option>
                    @endforeach
                </select>
                @error('pengajuan_id')
                ...
                @if($pengajuans->isEmpty())
                    <small class="text-muted">Tidak ada pengajuan yang tersedia untuk didistribusikan. Pastikan status pengajuan bukan 'disetujui' dan belum didistribusikan.</small>
                @endif
            </div>
            <div class="mb-3">
